...
PYBIND11_MODULE(freekv_cpp, m) {
  ...
  py::class_<BatchDecodeWithPagedKVCachePyTorchWrapper>(
	...
	m.def("echo_recall_dense_kv_from_starts_cuda", &echo_recall_dense_kv_from_starts_cuda);
	...
	m.def("echo_decode_qk_scores_pagemax_chunk_reduced", &echo_decode_qk_scores_pagemax_chunk_reduced);
