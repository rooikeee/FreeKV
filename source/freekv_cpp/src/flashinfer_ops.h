...
void echo_recall_dense_kv_from_starts_cuda(
    const torch::Tensor &starts_i32,   // [bsz or 1, n_pages], int32/int64, cuda
    const torch::Tensor &src_k_hsd,    // [bsz, n_kv_heads, src_tokens, head_dim], cuda
    const torch::Tensor &src_v_hsd,    // [bsz, n_kv_heads, src_tokens, head_dim], cuda
    torch::Tensor dst_k_hsd,           // [bsz, n_kv_heads, page_count*page_size, head_dim], cuda
    torch::Tensor dst_v_hsd,           // [bsz, n_kv_heads, page_count*page_size, head_dim], cuda
    int64_t page_size,
    int64_t page_begin,
    int64_t page_count,
    int64_t valid_tokens
