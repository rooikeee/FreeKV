...
RAFT_KERNEL SelectTopkKernel(const T *in,      // [bsz, len]
                             ...
                             T *new_in,        // [bsz, cap]
                             int32_t *incache, // [bsz, len + 1], cc2gp
                             IdxT *pos_ids,    // [bsz, cap], gc2cc
  ...
  if (n_recalls == 0)
    return;
  // new_in[i]: score of gpu cache page i
  for (int i = threadIdx.x; i < cap; i += blockDim.x) {
    new_in[i] = in[pos_ids[i]];
  }
  __syncthreads();
  // get top-n_recalls smallest scores of gpu cache pages
  // store index in out_idx[ns+n_incache:]
  radix_topk_one_block_kernel_helper<T, IdxT, BitsPerPass, BlockSize>(
      new_in + ns, nullptr, cap - ns - nw, n_recalls, out,
      out_idx + ns + n_incache, !select_min, bufs, counter, histogram);
  __syncthreads();
  // evict n_recall pages with smallest scores (out_idx[ns+n_incache]->)
