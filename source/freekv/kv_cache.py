...
class KvPool(PagePool):
    def __init__(
        ...
        self.layout = layout
        if layout != "NHD":
            assert layout == "HND" and device == torch.device("cpu")
            layout_map = (2, 0, 1, 3)
        else:
            assert layout_map == (0, 1, 2, 3)
        super().__init__(
            n_max_pages,
        ...
        if self.budget is None or self.n_real_pages + 1 <= self.budget:
            ...
            if self.n_win_pages > 1:
                e_gci = (
                    self.budget
                    - self.n_win_pages
                    + self._evict_cnt % (self.n_win_pages - 1)  # the last page is for allocation
                )
            else:
