...
def old_flash_attention_2_forward(
    ...
    if past_key_value is not None:
        ...
        if self.kv8:
            key_states = torch.cat([
                asym_dequant_int8(past_key_value[0], self.k_scale, self.k_zero, key_states.dtype), 
                key_states
            ], dim=1)#.contiguous()
            value_states = torch.cat([
                asym_dequant_int8(past_key_value[1], self.v_scale, self.v_zero, value_states.dtype), 
                value_states
            ], dim=1)#.contiguous()
        else:
    ...
    if self.kv8:
        if past_key_value is None:
            key_states, k_scale, k_zero = asym_quant_int8(key_states)
            value_states, v_scale, v_zero = asym_quant_int8(value_states)
            ...
            new_quant_k, k_scale, k_zero = asym_quant_int8(key_states[:, -q_len:, ...])
            new_quant_v, v_scale, v_zero = asym_quant_int8(value_states[:, -q_len:, ...])
            self.k_scale = torch.cat([self.k_scale, k_scale], dim=1)
            self.v_scale = torch.cat([self.v_scale, v_scale], dim=1)
            self.k_zero = torch.cat([self.k_zero, k_zero], dim=1)
...
def _flash_attention_forward(
    ...
    if padding_mask is not None:
        ...
        attn_output = flash_attn_maybe_npu(
            query_states,
            key_states,
            value_states,
            dropout,
...
def old_llama_model_forward(
    ...
    if position_ids is None:
        ...
        position_ids = position_ids.unsqueeze(0).repeat(batch_size, 1).view(-1, seq_length)
    else:
        position_ids = position_ids.view(-1, seq_length).long()

    device = input_ids.device if input_ids is not None else inputs_embeds.device
    indptr = torch.tensor(
        [i * seq_length for i in range(batch_size + 1)], dtype=torch.int32, device=device
    )
...
def enable_tuple_kv_cache_for_llama(model: LlamaForCausalLM, kv8=False):
    ...
    for idx in range(len(model.model.layers)):
        ...
        if not hasattr(model.model.layers[idx].self_attn, "layer_idx"):
